...
fun tags () {
    val line = "<html><head>Hello!<b><p>hello</head></html>"
    val pattern = """<[^<>/]+>"""
...
fun agfa () {
    val text = "agfa goes to rain tonight"
    val pattern = """\b[a-z]{4}\b"""
    val regex = pattern.toRegex()
...
fun isDate(date: String) : Boolean {
    // "12 МАР 2022" -> true
    val pattern = """\d{1,2}\s(ЯНВ|ФЕВ|МАР)\s\d{4}"""
    val regex = pattern.toRegex()

    for (result in regex.findAll(date)) {
